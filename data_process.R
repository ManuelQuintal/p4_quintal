...
dat <- read_csv("SalesBook_2013.csv") %>%
    ...
    filter(
        ...
        detachedGarage = gartype %>% str_to_lower() %>% str_detect("att") %>% as.numeric(),
        carportGarage = gartype %>% str_to_lower() %>% str_detect("att") %>% as.numeric(),
        noGarage = gartype %>% str_to_lower() %>% str_detect("att") %>% as.numeric(),
    ) %>%
